...
#### Error: Permisos / Permissions
En sistemas Unix/Linux, usar `sudo` si es necesario / On Unix/Linux, use `sudo` if needed:
```bash
...
```c
// Comentario de una línea / Single-line comment
/* 
...
### 2. Variables y Tipos de Datos / Variables and Data Types

```c
// Enteros / Integers
int edad = 25;             // Número entero / Whole number
long int poblacion = 8000000L;  // Entero largo / Long integer

// Punto flotante / Floating point
float promedio = 85.5f;
...
// Booleanos / Booleans
int es_verdadero = 1;  // true
int es_falso = 0;      // false
...
int igual = (a == b);     // Igual a / Equal to
int distinto = (a != b);  // Distinto de / Not equal to
int mayor = (a > b);      // Mayor que / Greater than
int menor = (a < b);      // Menor que / Less than

// Lógicos / Logical
int y_logico = (a > 0) && (b < 10);  // AND
int o_logico = (a > 0) || (b > 0);   // OR
int no_logico = !(a == b);           // NOT

// Asignación / Assignment
a += 5;    // Equivalente a: a = a + 5
a++;       // Incremento / Increment
a--;       // Decremento / Decrement
```

### 4. Estructuras de Control / Control Structures

```c
// If-Else
if (edad >= 18) {
    // Código si es mayor de edad / Code if adult
} else if (edad >= 13) {
...
switch (dia) {
    ...
    // ...
    default:
        print("Día no válido");  // Invalid day
}
...
// Función sin retorno / Void function
void saludar(char nombre[]) {
    print("Hola, ");
    print(nombre);
}

// Llamada a función / Function call
int resultado = sumar(5, 3);
saludar("Usuario");  // User
```
...
// Acceso / Access
int primer = numeros[0];  // 1
int ultimo = numeros[4];  // 5
...
usuario.edad = 25;
strcpy(usuario.nombre, "Juan");

// Inicialización / Initialization
struct Persona jefe = {"María", 35, 1.75f};
```
...
```c
int numero = 10;
int* puntero = &numero;  // Dirección de memoria / Memory address

// Acceso al valor / Value access
int valor = *puntero;  // 10

// Modificación a través del puntero / Modification through pointer
*puntero = 20;  // numero ahora es 20
```

### 9. Manejo de Archivos / File Handling

```c
// Escritura / Writing
FILE* archivo = fopen("datos.txt", "w");
fprintf(archivo, "Línea de texto\n");  // Text line
fclose(archivo);
...
### 10. Programa Completo / Complete Program
...
int main() {
    int num = 5;
    int fact = calcular_factorial(num);
...
Copyright (c) 2025 Equipo de Compiladores - UDB
...
uso, copia, modificación, fusión, publicar, distribuir, sublicenciar y/o vender
